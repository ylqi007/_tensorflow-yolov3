...
def darknet53(input_data, trainable):
    with tf.variable_scope('darknet'):
        ...
        for i in range(1):
            input_data = common.residual_block(input_data, 64, 32, 64,  # conv2, conv3
                                               trainable=trainable, name='residual%d' % (i + 0))
        input_data = common.convolutional(input_data, filters_shape=(3, 3, 64, 128),
                                          trainable=trainable, name='conv4', downsample=True)

        for i in range(2):
            input_data = common.residual_block(input_data, 128, 64, 128,    # conv5,6, conv7,8
